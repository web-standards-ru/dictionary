...
**изящная деградация** — подход, при котором интерфейс сайта *изящно деградирует* для старых браузеров, где не поддерживаются используемые технологии, по возможности смягчая ущерб опыту взаимодействия. Например: цвет подложки вместо градиента, растровая графика вместо векторной, проигрывание видео с помощью плагина.
...
**прогрессивное улучшение** — подход, при котором все браузеры получают только ограниченную базовую функциональность, а возможности в современных браузерах расширяются в соответствии с поддержкой новых технологий. Например: градиенты, векторная графика, воспроизведение видео.
...
**унифицированный указатель ресурса,** это [URI](#URI), который предоставляет ещё и информацию о местонахождении этого ресурса, напр. _background: url("http://www.example.com/picture.png")_
