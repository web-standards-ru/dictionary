...
**алиас,** альтернативное, чаще всего сокращенное имя команды или параметра команды. Применяется в терминале, командной строке, командной оболочке [гит](#git) для повышения читаемости и скорости набора команд. Например, вместо написания полной команды `git commit` можно использовать алиас — `git ci`. Для многих параметров команд (аргументов командной строки) уже заданы алиасы по-умолчанию. Например, можно использовать команду `git commit -m` вместо `git commit --message`.
