...
**куки,** небольшой фрагмент данных, отправляемый сервером и сохраняемый браузером пользователя. Эти данные отправляются обратно с каждым новым запросом к данному серверу. В единственном числе — **кука**. Примеры склонений: _доступ к кукам, Директива Евросоюза о куках, сторонними куками, авторизационная кука_.
