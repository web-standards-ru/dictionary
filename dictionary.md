...
**правильность,** свойство в системах матлогики, указывающее на то, что каждая формула в рамках системы является логически валидной. В компьютерных науках часто употребляется в контексте системы типов. Напр. _sound types system - правильная система типов_, система типов в коде, позволяющая исполнять его без ошибок, [см. типобезопасность](https://ru.wikipedia.org/wiki/Типобезопасность).
