...
**идентификатор фрагмента,** текст после `#` в URL, позволяющий адресовать конкретную часть документа. Данный термин используется в основном в спецификацией и при работе с SVG, в HTML устоялся более привычный термин [якорь](#anchor).
