...
**Кастомные свойства** (возм. **css-переменные**) - свойства, начинающиеся с двух дефис, которые разработчик может задавать и использовать их значения как переменные. Например, код `--c: 1px;` задает переменную `--c` со значением `1px`. Использовать ее можно как `var(--c)`.
