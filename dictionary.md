...
**клиарфикс,** приём для предотвращения схлопывания контейнеров с плавающими блоками. Приём заключается в создании специальной распорки, которая располагается после плавающих блоков и отменяет их обтекание. Важно понимать, что клиарфикс — лишь частный случай решения одной из проблем, вызываемых флоатами (см. [сводную таблицу всех проблем и их решений](http://css-live.ru/articles-css/clearfix-block-formatting-context-methods-cheatsheet.html)).
